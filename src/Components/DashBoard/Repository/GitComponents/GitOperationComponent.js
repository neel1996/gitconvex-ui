...
export default function GitOperationComponent() {
  ...
  function getTableData() {
    ...
    let actionButton = (
      <div className="p-2  bg-green-300 text-white rounded-md shadow-sm hover:shadow-md">
        Add
      </div>
    );
    ...
    gitUntrackedFiles.forEach((item) => {
      ...
      if (item[0]) {
        item[0].split(",")
          ...
          : tableDataArray.push([item[0], statusPill("N"), actionButton]);
      }
    });
      ...
      <table className="table border-0 w-full cursor-pointer" cellPadding="20">
        ...
        <tbody>
          {getTableData().map((tableData, index) => {
            return (
              <tr
                className="m-4 text-xl font-sans border-b border-gray-300"
                key={`tableItem-${index}`}
              >
                {tableData.map((data) => {
