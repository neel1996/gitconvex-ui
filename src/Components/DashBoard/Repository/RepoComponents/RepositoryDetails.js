...
export default function RepositoryDetails(props) {
  ...
  const gitTrackedFileComponent = () => {
    const fetchFolderContent = (directoryName) => {
      if (repoIdState && directoryName) {
        setGitRepoFiles([]);
        ...
        axios({
          ...
          .then((res) => {
            if (res.data.data && !res.data.error) {
              ...
              setGitRepoFiles([...localFolderContent.gitTrackedFiles]);
              setGitFileBasedCommits([
    ...
    if (
      ...
      gitRepoFiles.forEach((entry, index) => {
        ...
        if (splitEntry[1].includes("directory")) {
          let directorypath = splitEntry[0];
            ...
            >
              <div className="flex cursor-pointer">
                ...
                <div
                  ...
                  onClick={(event) => {
                    fetchFolderContent(splitEntry[0]);
                  }}
                >
                  {directorypath}
                </div>
